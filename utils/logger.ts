...
type LogLevel = 'LOG' | 'INFO' | 'START' | 'SUCCESS' | 'WARN' | 'ERROR' | 'MIDDLEWARE';

const logStyles: Record<LogLevel, { emoji: string; color: string }> = {
  LOG: { emoji: '📝', color: '\x1b[37m' },     // White (default)
  INFO: { emoji: 'ℹ️', color: '\x1b[36m' },     // Cyan (process information)
  ...
  MIDDLEWARE: { emoji: '⚙️', color: '\x1b[35m' }, // Purple (middleware operations)
};
...
export const createLogger = (context: string) => {
  ...
  const getTimestamp = () => {
    ...
    const formattedTime = timePassed >= 1000
  ...
  const isContextHidden = () => config.useHiddenContexts && config.hiddenContexts.includes(context);
  ...
  const logMessage = (level: LogLevel, ...args: any[]) => {
    if (config.enabled && !isContextIgnored()) {
      ...
      const message = `${color}${timestamp} | ${paddedContext} | ${emoji} | ${formattedArgs.join(' ')}\x1b[0m`;
